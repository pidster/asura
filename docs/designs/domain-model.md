...
Status: proposed semantic model for review. Existing requirements retain their
authority through the linked contracts. No physical schema, runtime mechanism or
...
independently registered worktree record.
...
Proposed behavior: a conversation belongs to one project context and principal
...
task revisions, location validation and atomic admission. D4 owns observation
schemas, action granularity and graph reuse. D5 owns remote identity mappings;
...
or accept these proposed relationships. Before implementation, D3-D4 must provide
executable contracts and tests for aliasing, overlap, retries and session isolation.
